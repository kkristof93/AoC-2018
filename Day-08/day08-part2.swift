...
func getValue(_ nodeNumber: Int) -> Int {
	let node = tree[nodeNumber]

	if node.childCount == 0 {
		return node.metadata.reduce(0, +)
	}

	let values = node.metadata.compactMap { 
		($0 <= node.children.count && $0 > 0) ? getValue(node.children[$0 - 1]) : nil 
	}

	return values.reduce(0, +)
}

let rootValue = getValue(0)
print(rootValue)
